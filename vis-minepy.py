...
def mysubplot_MINE(x, y, numRows, numCols, plotNum,
              xlim=(-4, 4), ylim=(-4, 4)):
...
def mvnormal(n=1000):
    ...
    for i, cor in enumerate(cors):
        ...
        mysubplot_MINE(xy[:, 0], xy[:, 1], 3, 7, i+1)
...
def rotnormal(n=1000):
    ...
    for i, t in enumerate(ts):
        xy_r = rotation(xy, t)
        mysubplot_MINE(xy_r[:, 0], xy_r[:, 1], 3, 7, i+8)

def others(n=1000):
    x = rs.uniform(-1, 1, n)
    y = 4*(x**2-0.5)**2 + rs.uniform(-1, 1, n)/3
    mysubplot_MINE(x, y, 3, 7, 15, (-1, 1), (-1/3, 1+1/3))

    y = rs.uniform(-1, 1, n)
    xy = np.concatenate((x.reshape(-1, 1), y.reshape(-1, 1)), axis=1)
    xy = rotation(xy, -np.pi/8)
    lim = np.sqrt(2+np.sqrt(2)) / np.sqrt(2)
    mysubplot_MINE(xy[:, 0], xy[:, 1], 3, 7, 16, (-lim, lim), (-lim, lim))

    xy = rotation(xy, -np.pi/8)
    lim = np.sqrt(2)
    mysubplot_MINE(xy[:, 0], xy[:, 1], 3, 7, 17, (-lim, lim), (-lim, lim))

    y = 2*x**2 + rs.uniform(-1, 1, n)
    mysubplot_MINE(x, y, 3, 7, 18, (-1, 1), (-1, 3))

    y = (x**2 + rs.uniform(0, 0.5, n)) * \
        np.array([-1, 1])[rs.random_integers(0, 1, size=n)]
    mysubplot_MINE(x, y, 3, 7, 19, (-1.5, 1.5), (-1.5, 1.5))

    y = np.cos(x * np.pi) + rs.uniform(0, 1/8, n)
    x = np.sin(x * np.pi) + rs.uniform(0, 1/8, n)
    mysubplot_MINE(x, y, 3, 7, 20, (-1.5, 1.5), (-1.5, 1.5))
    ...
    mysubplot_MINE(xy[:, 0], xy[:, 1], 3, 7, 21, (-7, 7), (-7, 7))
